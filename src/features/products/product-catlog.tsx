...
export function ProductCatalog() {
  ...
  const limit = 20;
  const { data, isLoading, isError, error } = useProductCatalog({ page, limit, searchKey });
                ...
                <TableBody>
                  {data?.data.map((p) => (
                    <TableRow key={p._id}>
                      <TableCell>
                        {p.productImage ? (
                          <img src={p.productImage} loading="lazy" alt={p.productName} className="h-12 w-12 rounded object-cover" />
                        ) : (
